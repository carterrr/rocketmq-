...
public class CommitLog {
    ...
    public CommitLog(final DefaultMessageStore defaultMessageStore) {
        ...
        if (FlushDiskType.SYNC_FLUSH == defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            // 同步刷盘  用 GroupCommitService  的run  来提交
            this.flushCommitLogService = new GroupCommitService();
        } else {
            // 2. 异步刷盘时 用的是 FlushRealTimeService
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        //1. 这里面最终会走到commitlog.putmessage()方法 执行真正的提交流程 参考消息存储流程逻辑
        CompletableFuture<PutMessageStatus> flushResultFuture = submitFlushRequest(result, putMessageResult, msg);
        //2. 发送到slave的逻辑 拿到一个future 等待返回
        CompletableFuture<PutMessageStatus> replicaResultFuture = submitReplicaRequest(result, putMessageResult, msg);
        // 6. 两个future都返回了 再将结果写到result中 这里是被两个future都执行complete方法才触发的 注意第五点那里的complete触发逻辑
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            || tranType == MessageSysFlag.TRANSACTION_COMMIT_TYPE) {
            // 6. 延迟级别大于0 说明是延时消息或消息重试 把消息的真实主题和queueid存储到消息属性中 并修改为延时topic 和延时 queueid
            if (msg.getDelayTimeLevel() > 0) {
                if (msg.getDelayTimeLevel() > this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel()) {
                    msg.setDelayTimeLevel(this.defaultMessageStore.getScheduleMessageService().getMaxDelayLevel());
                }
        ...
        long elapsedTimeInLock = 0;
        // 7. 拿到最后一个commitlog文件 并加锁  mappedFileQueue对应commitlog文件夹 MappedFile是commitlog文件夹下的单个文件
        MappedFile unlockMappedFile = null;
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();
        ...
        try {
            ...
            // global
            msg.setStoreTimestamp(beginLockTimestamp);
            // 空的 （没有创建过commitlog文件）或 满了 （应该写下一个文件了）用偏移量0创建一个文件
            if (null == mappedFile || mappedFile.isFull()) {
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }
            if (null == mappedFile) {
            ...
            switch (result.getStatus()) {
                case PUT_OK:
                    break;
                case END_OF_FILE:  // 文件大小不够 这里创建新的文件
                    unlockMappedFile = mappedFile;
                    // Create a new file, re-write the message
                    mappedFile = this.mappedFileQueue.getLastMappedFile(0); // 创建新文件
                    if (null == mappedFile) {
                        // XXX: warn and notify me
                        log.error("create mapped file2 error, topic: " + msg.getTopic() + " clientAddr: " + msg.getBornHostString());
                        beginTimeInLock = 0;
        ...
        storeStatsService.getSinglePutMessageTopicTimesTotal(msg.getTopic()).incrementAndGet();
        storeStatsService.getSinglePutMessageTopicSizeTotal(topic).addAndGet(result.getWroteBytes());
        // 17. 依据刷盘模式进行同步刷盘 or 异步刷盘
        handleDiskFlush(result, putMessageResult, msg);
        // 18. 如果是master判断是否需要同步双写（需要等待主从同步完成）并进行同步双写
    ...
    public CompletableFuture<PutMessageStatus> submitFlushRequest(AppendMessageResult result, PutMessageResult putMessageResult,
        ...
        // Asynchronous flush
            ...
            // 异步刷盘  开启  TransientStorePoolEnable 时 先 （参考 CommitLog初始化方法  ）
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        //                  该方法中读取判断开启了  TransientStorePoolEnable  会将  fileChannel 中内容刷盘
        //         否则
        //               直接 走  flushCommitLogService 的run方法    该方法中读取判断没有开启   TransientStorePoolEnable
        //                   直接会 异步 将 MappedByteBuffer 中的内容刷到磁盘
        else {
            if (!this.defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                // TransientStorePoolEnable为false
                flushCommitLogService.wakeup();
            } else {
                // 3. TransientStorePoolEnable为true时  对应类是 CommitRealTimeService
                commitLogService.wakeup();
            }
        }
    }
    ...
    public PutMessageResult putMessages(final MessageExtBatch messageExtBatch) {
        ...
        storeStatsService.getSinglePutMessageTopicTimesTotal(messageExtBatch.getTopic()).addAndGet(result.getMsgNum());
        storeStatsService.getSinglePutMessageTopicSizeTotal(messageExtBatch.getTopic()).addAndGet(result.getWroteBytes());
        // MARK 从 byteBuffer 刷盘  前边代码都是将message 放到MappedByteBUffer  内存中
    ...
    // 每隔200ms追加堆外内存ByteBuffer到FileChannel中的线程
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 一次至少提交到 fileChannel 的页数  默认4页
                int commitDataLeastPages = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getCommitCommitLogLeastPages();
                // 两次真实提交的最大间隔 默认 200ms
                ...
                if (begin >= (this.lastCommitTimestamp + commitDataThoroughInterval)) {
                    ...
                    // 时间已经过了 两次真实提交的最大间隔 无视页数限制  直接提交
                    commitDataLeastPages = 0;
                }

                try {
                    //4. 执行提交操作  将 byteBuffer 中提交相应页数  为0提交全部页 到物理文件的内存映射fileChannel
                    boolean result = CommitLog.this.mappedFileQueue.commit(commitDataLeastPages);
                    long end = System.currentTimeMillis();
                    if (!result) { // false表示提交成功  这里有点绕
                        this.lastCommitTimestamp = end; // result = false means some data committed.
                        //now wake up flush thread.
                        //5. 提交成功  唤醒刷盘线程  对应会运行类 FlushRealTimeService 的 run（） 方法
                        flushCommitLogService.wakeup();
                    }
    ...
    // 每隔 500ms将物理文件对应内存映射MappedByteBuffer 或 fileChannel  刷到 硬盘 上
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                // 是否使用wait 方法等待   默认false  就是用Thread.sleep()方法等待
                boolean flushCommitLogTimed = CommitLog.this.defaultMessageStore.getMessageStoreConfig().isFlushCommitLogTimed();
                // 从MappedByteBuffer 或 fileChannel  刷到 磁盘的时间间隔 也就是本线程运行的时间间隔
                int interval = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushIntervalCommitLog();
                // 最少刷盘页数 默认4页
                int flushPhysicQueueLeastPages = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushCommitLogLeastPages();
                // 两次刷盘任务最大时间间隔 默认10秒  超过不论多少页 即使小于最小刷盘页数4 也直接刷盘
                int flushPhysicQueueThoroughInterval =
                    CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushCommitLogThoroughInterval();

                boolean printFlushProgress = false;

                // Print flush progress  距离上次刷盘时间超过 flushPhysicQueueThoroughInterval 不考虑页数 直接刷到磁盘
                ...
                try {
                    ...
                    if (printFlushProgress) {
                        this.printFlushProgress(); // 一定次数后才打印日志
                    }
                    ...
                    if (storeTimestamp > 0) {
                        //7. 更新checkpoint 时间戳  更新逻辑在DefaultMessageStore#FlushConsumeQueueService 也是一个独立线程
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // PHY OFFSET  当前的写位置  byteBuffer是没有刷盘的内存记录
            ...
            String msgId;
            // 10. 设置全局唯一消息id 4字节ip + 4字节端口号 + 8字节消息偏移量  UtilAll.string2bytes方法能将msgId字符串还原成16字节的数组  根据提取的消息物理偏移量
            if ((sysflag & MessageSysFlag.STOREHOSTADDRESS_V6_FLAG) == 0) {
            ...
            // 13. 如果offset超过了文件1GB的限制 返回需要创建新commitLog文件
            ...
            // byteBuffer中写入 0 到  msgLen 长度的消息比特数据
            // 14. 将消息追加到内存中 并创建AppendMessageResult对象  后续步骤依据同步刷盘还是异步刷盘模式写到磁盘中
